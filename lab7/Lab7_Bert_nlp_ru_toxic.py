...
df = pd.read_csv("./data/dataset.csv", delimiter=',', header=0, names=['sentence', 'label'])
...
test_sentences = [
    'Фильм вызывает неоднозначные эмоции если было бы чуть больше активности фильм заслуживал бы оскара, а пока полный шлак'
]
...
for res in results:
    print(f"Класс {res['predicted_class']} ({res['confidence'].item():.2%}): {res['sentence']}")
